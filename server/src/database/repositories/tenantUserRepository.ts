...
import { invitationTemplate } from "../../../email-templates/invitationTemplate.html";
...
export default class TenantUserRepository {
  ...
  static async acceptInvitation(invitationToken, options: IRepositoryOptions) {
    ...
    let existingTenantUser = currentUser.tenants.find(
      (userTenant) =>
        String(userTenant.tenant.id) === String(invitationTenantUser.tenant.id)
    );
    ...
    const emailVerified =
      currentUser.emailVerified || isSameEmailFromInvitation;
...
function selectStatus(oldStatus, newRoles) {
  ...
  if (!newRoles.length || oldStatus === "empty-permissions") {
