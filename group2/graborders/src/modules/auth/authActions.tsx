...
const authActions = {
  ...
  doRegisterEmailAndPassword: (email, password,invitationcode) => async (dispatch) => {
    try {
      dispatch({ type: authActions.AUTH_START });

      const token = await service.registerWithEmailAndPassword(email, password,invitationcode);
